...
export default function CityCategoryPage() {
  ...
  }, [debouncedQuery, sort, citySlug, categorySlug])
  ...
  async function fetchResults(pageNum: number, reset = false) {
    ...
    try {
      ...
      else params.set('q', `${categoryData?.name || categorySlug} in ${cityData.name}`)
      if (categoryData?.slug) params.set('type', categoryData.slug)
  ...
  const categoryName = categoryData?.name ?? categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1)
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 pt-4 pb-3 flex flex-col gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={`Search ${categoryName.toLowerCase()} in ${cityData.name}…`}
          ...
          {!loading && results.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-muted max-w-xs">
                No {categoryName.toLowerCase()} found in {cityData.name}. Try broadening your search.
              </p>
            </div>
          )}
        </div>
